...
class VendingMachine
  ...
  # ステップ４　機能拡張
  # ジュースを3種類管理できるようにする。
  ITEMS_PRICE={cola: 120, redbull: 200, water: 100}.freeze
  ...
  def can_buy?(item)
    if ITEMS_PRICE.has_key?(item)
      ( (@slot_money>=ITEMS_PRICE[item]) && (@stock[item] != nil && @stock[item]>0) ) ? true : false
    else
      false
    end
  end
